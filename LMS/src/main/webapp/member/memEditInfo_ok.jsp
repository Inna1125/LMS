...
<%
			...
			try{
				String sql = "update member set m_name =?, m_passwd =?, m_zipNo =?, m_addr =?, m_addrDetail =?, m_tel =?, m_phone =?, m_email =?, m_sendOk =?, m_dept =?, m_position =?, c_code =? where m_id =?";
				pstmt = conn.prepareStatement(sql);
				
				pstmt.setString(1,m_name);
				pstmt.setString(2,m_passwd);
				pstmt.setString(3,m_zipNo);
				pstmt.setString(4,m_addr);
				pstmt.setString(5,m_addrDetail);
				pstmt.setString(6,m_tel);
				pstmt.setString(7,m_phone);
				pstmt.setString(8,m_email);
				pstmt.setString(9,m_sendOk);
				pstmt.setString(10,m_dept);
				pstmt.setString(11,m_position);
				pstmt.setString(12,c_code);
				pstmt.setString(13,m_id);
				pstmt.executeUpdate();
				
%>
				<script>
					alert("회원정보수정이 완료되었습니다");
					location.href = "memIndex.jsp";
				</script>
<%
