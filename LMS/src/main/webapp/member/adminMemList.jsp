...
<body>
	...
	<div class="content">
		<h2>회원목록</h2>
		<hr>
	<form name = "search" method = "get" action = "adminSearch_ok.jsp" class = "searchfrm">
		<select name = "method">
			<option value = "">선택하세요.</option>
			<option value = "m_name">이름</option>
			<option value = "c_code">소속회사</option>
		</select>
	...
	<table width = "1200px">
		...
		try{
			...
			sql = "select m.m_no, m.m_name, c.c_name,m.m_dept, m.m_position, m.m_phone, m.m_count from member m join company c on m.c_code = c.c_code order by m_no desc limit "+idxNo+","+unit;
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
		...
		<tr>
			...
			<td><a href = # onclick = "window.open('adminMemDetail.jsp?m_no=<%=m_no%>', 'pop', 'width=800px, height=1000px, top=0, left=0 resizable=yes')"><%=m_name%></a></td>
			<td><%=c_name%></td>
			<td><%=m_dept%></td>
			<td><%=m_position%></td>
			<td><%=m_phone%></td>
	...
	<div class = "page" style="text-align: center;">
		<ul class = "justify-content-center">
			...
			<%}else{%>
			<li class = "page-item"><a class = "page-link" href="adminMemList.jsp?cPage=<%=startPage-1%>" tabindex="-1" aria-disabled="true">Previous</a></li>
			<%}%>
			<%for(int p = startPage;p<=endPage;p++){ %>
			<li class = "page-item">
				<a class = "page-link" href = "adminMemList.jsp?cPage=<%=p%>"><%=p %></a></li>
			<%}%>
			<%if(totalPage == endPage){ %>
			<li class = "page-item disabled"><a class = "page-link" href ="#">Next</a></li>
			<%}else{ %>
			<li class = "page-item">
				<a class = "page-link" href = "adminMemList?cPage=<%=endPage+1%>">Next</a>
			</li>
			<%} %>
		</ul>
<%
