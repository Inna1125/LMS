...
<body>
		...
		<%
			...
			try{
				...
				while(rs.next()){
					...
					%>
						<li><a href = "categoryList.jsp?b_categoryF=<%=b_categoryF%>">
							<% switch(b_categoryF){
								case "3A" : 
									out.println("자기계발");
									break;
	...
	<div style="width: 600px; text-align: center; margin-top: 10px;" class = "page">
		<ul class = "justify-content-center">
			...
			<%}else{%>
			<li class = "page-item"><a class = "page-link" href="categoryListBook.jsp?cPage=<%=startPage-1%>" tabindex="-1" aria-disabled="true">Previous</a></li>
			<%}%>
			<%for(int p = startPage;p<=endPage;p++){ %>
			<li class = "page-item">
				<a class = "page-link" href = "categoryListBook.jsp?cPage=<%=p%>"><%=p %></a></li>
			<%}%>
			<%if(totalPage == endPage){ %>
			<li class = "page-item disabled"><a class = "page-link" href ="#">Next</a></li>
			<%}else{ %>
			<li class = "page-item">
				<a class = "page-link" href = "categoryListBook.jsp?cPage=<%=endPage+1%>">Next</a>
			</li>
			<%} %>
		</ul>
		<%
