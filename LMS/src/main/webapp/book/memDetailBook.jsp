...
<div class = "detail">
	...
	%>		
			<form method = "post" name = "bookfrm" enctype = "multipart/form-data" action = "../heart/memHeartInsert_ok.jsp?b_no=<%=b_no%>&m_id=<%=m_id%>">
			<fieldset>
				<table>
				<tr>
	...
	%>
				<p><input type = "button" class = "but" onclick = "window.close()" value = "닫기">
				<%if(((String)session.getAttribute("m_id")).equals("admin")){ %>
					<input type = "button" class = "but" onclick = "location.href='adminEditBook.jsp?b_no=<%=b_no%>'" value = "수정하기">
				<%}else{%>
					<input type = "submit" class = "but" onclick = heart() value = "찜하기">
				<%}%>
			</form>
		</div>
</body>
