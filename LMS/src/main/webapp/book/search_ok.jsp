...
%>
	...
	<%		
			...
			PreparedStatement pstmt = null;
		 	...
		 	String sql = "select count(*) from book where " + method + " LIKE ?";
		...
		try{
			sql = "select b_no, b_code, b_image, b_name, date_format(b_date,'%Y.%m.%d')as b_date, b_price, b_pub, date_format(b_regDate,'%Y.%m.%d')as b_regDate from book where " + method + " LIKE ? order by b_no desc limit "+idxNo+","+unit;
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%");
			rs = pstmt.executeQuery();
			while(rs.next()){
		...
		%>
				<tr>
					...
					<td><a href = # onclick = "window.open('detailBook.jsp?b_no=<%=b_no%>', 'pop','width=600, height=900, top=0, left=0 resizable=yes')"><%=b_name %></a></td>
					<td><%=b_date%></td>
					<td><%=b_price%></td>
					<td><%=b_pub%></td>
					<td><%=b_regDate%></td>
	...
	<div style="width: 600px; text-align: center; margin-top: 10px;" class = "page">
		<ul class = "justify-content-center">
			...
			<%}else{%>
			<li class = "page-item"><a class = "page-link" href="search_ok.jsp?cPage=<%=startPage-1%>&method=<%=method%>&keyword=<%=keyword%>" tabindex="-1" aria-disabled="true">Previous</a></li>
			<%}%>
			<%for(int p = startPage;p<=endPage;p++){ %>
			<li class = "page-item">
				<a class = "page-link" href = "search_ok.jsp?cPage=<%=p%>&method=<%=method%>&keyword=<%=keyword%>"><%=p %></a></li>
			<%}%>
			<%if(totalPage == endPage){ %>
			<li class = "page-item disabled"><a class = "page-link" href ="#">Next</a></li>
			<%}else{ %>
			<li class = "page-item">
				<a class = "page-link" href = "search_ok.jsp?cPage=<%=endPage+1%>&method=<%=method%>&keyword=<%=keyword%>">Next</a>
			</li>
			<%} %>
		</ul>
	</div>
	<a href = "listBook.jsp" class="backList">목록가기</a>
	</div>
