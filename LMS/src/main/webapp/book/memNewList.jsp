...
<body>
	...
	<table>
		...
		try {
			...
			try {
				sql = "select b_no, b_code, b_image, b_name, date_format(b_date,'%Y.%m.%d')as b_date, b_price, b_pub, date_format(b_regDate,'%Y.%m.%d')as b_regDate from book order by b_date desc limit "+idxNo+"," + unit;
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
				while (rs.next()) {
	...
	<div style="text-align: center; margin-top: 10px;" class = "page">
		<ul class = "justify-content-center">
			...
			<%}else{%>
			<li class = "page-item"><a class = "page-link" href="memNewList.jsp?cPage=<%=startPage-1%>" tabindex="-1" aria-disabled="true">Previous</a></li>
			<%}%>
			<%for(int p = startPage;p<=endPage;p++){ %>
			<li class = "page-item">
				<a class = "page-link" href = "memNewList.jsp?cPage=<%=p%>"><%=p %></a></li>
			<%}%>
			<%if(totalPage == endPage){ %>
			<li class = "page-item disabled"><a class = "page-link" href ="#">Next</a></li>
			<%}else{ %>
			<li class = "page-item">
				<a class = "page-link" href = "memNewList.jsp?cPage=<%=endPage+1%>">Next</a>
			</li>
			<%} %>
		</ul>
		<%
