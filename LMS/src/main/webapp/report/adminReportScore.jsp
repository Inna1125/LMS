...
<%@include file = "../Menu/adminHeader.jsp" %>
	<div class = "context">
		...
		<hr>
	<div class = "searchfrm">
		<form name = "search" method = "get" action = "adminSearch_ok.jsp">
			<select name = "method">
			...
			<input type="button" value="검색" onclick="check()" class = "searchBut">
		</form>
	</div>
	<div> 
		<form name = "reportfrm" method = "get" action = "adminReportDelete.jsp">
			<table>
				<tr>
				...
				<%
					...
					try{
						...
						try{
							sql = "select r.r_no, b.b_name, b.b_author, m.c_code, m.m_name, r.r_date, r.r_ip, r.r_done, r.r_score from report r join member m on r.m_no = m.m_no join book b on r.b_no = b.b_no order by r_no desc limit " + idxNo + " , " + unit;
							pstmt = conn.prepareStatement(sql);
							rs = pstmt.executeQuery();
							while(rs.next()){
							...
							sql = "select c_name from company where c_code =?";
							pstmt = conn.prepareStatement(sql);
	...
	%>
					<tr>
						...
						<td><%=i %></td>
						<td><a href = # onclick = "window.open('adminReportDetail.jsp?r_no=<%=r_no%>', 'pop', 'width=800px, height=1000px, top=0, left=0 resizable=yes')"><%=b_name %></a></td>
						<td><%=b_author %></td>
						<td><%=c_name %></td>
						<td><%=m_name %></td>
						<td><%=r_date %></td>
